...
if [ ! -d "$SYNC_SKILL_SRC" ]; then
  fail "Sync skill source not found at $SYNC_SKILL_SRC."
  exit 1
fi
if [ ! -f "$RULE_SRC" ]; then
...
# Symlink the sync skill ------------------------------------------------------

bold "2. Sync skill"
if [ -L "$SYNC_SKILL_DEST" ]; then
  CURRENT_TARGET="$(readlink "$SYNC_SKILL_DEST")"
  if [ "$CURRENT_TARGET" = "$SYNC_SKILL_SRC" ]; then
    skip "Symlink already points to this repo: $SYNC_SKILL_DEST"
  else
    rm "$SYNC_SKILL_DEST"
    ln -s "$SYNC_SKILL_SRC" "$SYNC_SKILL_DEST"
    ok "Replaced symlink: $SYNC_SKILL_DEST → $SYNC_SKILL_SRC (was → $CURRENT_TARGET)"
  fi
elif [ -e "$SYNC_SKILL_DEST" ]; then
  fail "Refusing to overwrite existing non-symlink at $SYNC_SKILL_DEST"
  echo "       Move it aside ('mv $SYNC_SKILL_DEST ${SYNC_SKILL_DEST}.bak') and re-run."
  exit 1
else
  ln -s "$SYNC_SKILL_SRC" "$SYNC_SKILL_DEST"
  ok "Symlinked: $SYNC_SKILL_DEST → $SYNC_SKILL_SRC"
fi
...
bold "4. Analytics data directory"
...
echo
echo "Next steps:"
echo "  1. Restart Cursor so it picks up the new skills + rule."
echo "  2. In any repo, ask Cursor: \"Bootstrap agent context for this repo.\""
echo "  3. Later, to pull pipeline updates into a bootstrapped repo:"
echo "       \"Sync agent context for this repo.\""
echo "  4. After your first convoy runs, see 'analytics/README.md' for self-analytics."
echo
