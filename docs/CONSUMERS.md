...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| [`zest`](https://github.com/varutasu/zest-finances) (private) | Personal | Next.js (formerly + Prisma — see drift) | Coolify | `nextjs-prisma-coolify/` ✅ migrated; **drift candidate** — Prisma removed since sync | L1 + L2 + L3 | upstream | `0.7.0` | 2026-08-10 | Synced v0.7 model routing (Composer architect, Grok audits). |
| [`tcg-vault`](https://github.com/stwl-labs/tcg-vault) (private, **product = Deck Hearth**) | Personal | Next.js 16 + Drizzle (Postgres) + React 18 | Vercel | Mixed: `nextjs/` base + selective `nextjs-prisma-vercel/` borrows (`pr-health-rollup.yml`, `preview-smoke.yml`, `visual-diff.yml`, `flags-index.ts`). Needs cleanup — strictly should be on a new `nextjs-vercel/` variant (does not yet exist) OR `nextjs/` with Vercel-aware preview workflows backported. | L1 + L2 + L3 | upstream | `0.7.0` | 2026-08-10 | Synced v0.7 model routing. **Still held back:** `pr-health-rollup.yml`, `ci.yml`, `CODEOWNERS`, `no-go-zones.mdc`, `api-routes.mdc`, preview/visual workflows. |
