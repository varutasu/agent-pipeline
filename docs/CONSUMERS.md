...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| [`zest`](https://github.com/varutasu/zest) (private) | Personal | Next.js + Prisma | Coolify | `nextjs-prisma-coolify/` (currently on baseline) | L1 + L2 + L3 | upstream | `0.3.0` | 2026-05-21 | First end-to-end sync. Retroactive manifest. Step 5 installed `scripts/log-convoy-event.sh`. 22 artifacts tracked. `add-api-route` skill is repo-local (not in pipeline). **0.5.0 migration pending**: drop the `Build` step from `ci.yml`, delete `preview-smoke.yml` + `visual-diff.yml` (no per-PR previews). |
