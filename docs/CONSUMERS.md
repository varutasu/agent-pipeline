...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| [`zest`](https://github.com/varutasu/zest-finances) (private) | Personal | Next.js (formerly + Prisma — see drift) | Coolify | `nextjs-prisma-coolify/` ✅ migrated; **drift candidate** — Prisma removed since sync | L1 + L2 + L3 | upstream | `0.6.0` | 2026-06-23 | Phase 5.1 synced (`role-security-auditor`, security rules, skill, 4-way fan-out). **Still untracked in git** per prior note — commit when ready. No `pr-health-rollup.yml` installed. |
| [`tcg-vault`](https://github.com/stwl-labs/tcg-vault) (private, **product = Deck Hearth**) | Personal | Next.js 16 + Drizzle (Postgres) + React 18 | Vercel | Mixed: `nextjs/` base + selective `nextjs-prisma-vercel/` borrows (`pr-health-rollup.yml`, `preview-smoke.yml`, `visual-diff.yml`, `flags-index.ts`). Needs cleanup — strictly should be on a new `nextjs-vercel/` variant (does not yet exist) OR `nextjs/` with Vercel-aware preview workflows backported. | L1 + L2 + L3 | upstream | `0.6.0` | 2026-06-23 | Phase 5.1 partial sync: L2 roles + security artifacts updated. **Held back (manual merge):** `pr-health-rollup.yml`, `ci.yml`, `CODEOWNERS`, `no-go-zones.mdc`, `api-routes.mdc`, preview/visual workflows — see `*.pipeline-0.6.0` siblings. Repo-local rules/skills (`auth-and-permissions`, `db-and-schema`, `ui-and-theming`, `add-api-route`, `add-page`) intentionally untouched. `forbidden-patterns` CI is repo-local hardening. |
| [`localeloop`](https://github.com/rstillwell-trimb/localeloop) (private) | Trimble | Next.js + Prisma | Vercel | `nextjs-prisma-vercel/` (currently on baseline) | L1 + L2 + L3 | upstream (synced from) | `0.6.0` | 2026-06-23 | Synced after upstream Phase 5.1 (`role-security-auditor`, `security-baseline.mdc`, `convoy-planning.mdc`, `security-audit` skill, 4-way audit fan-out). PR template + convoys README updated. **0.5.0 migration still pending** for `ci.yml` / preview-smoke / visual-diff. |
| [`colab`](https://github.com/rstillwell-trimb/colab) (private) | Trimble | Next.js + Prisma | Cloud Build | `nextjs-prisma-cloudbuild/` (already aligned — workflows excluded from manifest) | L0 + L1 + L2 + L3 | upstream (synced from) | `0.6.0` | 2026-06-23 | Synced after upstream Phase 5.1. Added `role-security-auditor`, `security-baseline.mdc`, `convoy-planning.mdc`, `security-audit` skill; updated L2 roles + PR template + convoys README. `docs/agent-context/README.md` held back (colab-customized) — see `.pipeline-0.6.0` sibling. CODEOWNERS still pinned. GHA workflows still out-of-pipeline (Cloud Build). |
...
| [`survey-platform`](https://github.com/stwl-labs/survey-platform) (private) | Personal | Next.js | L1 (11 rules, 4 skills) + L3 (1 workflow) | upstream | 2026-06-23 | Phase 5.1 synced (security role + rules + skill). Partial install — no full L3 convoy stack. Likely wants L2 roles added via bootstrap if convoy workflow is desired. |
| [`tavernlight`](https://github.com/stwl-labs/Tavernlight-fn) (private) | Personal | unknown | L1 (7 rules) + 1 workflow + AGENTS.md | upstream | 2026-06-23 | Phase 5.1 partial sync. Held back: `docs/model-routing-policy.md` (local customization), `scripts/log-convoy-event.sh` (customized). |
| [`echo-board`](https://github.com/stwl-labs/echo-board) (private) | Personal | unknown | AGENTS.md only | upstream | 2026-06-23 | Phase 5.1 synced (security role + rules + skill + model routing). Still bare on L3 convoy infra. |
